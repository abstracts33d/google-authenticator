    ...
    context 'integration with ActiveRecord' do
      ...
      context 'custom secret column' do
        ...
        it 'generates a url for a qr code' do
          @user.google_qr_uri.should == "https://image-charts.com/chart?cht=qr&chl=otpauth%3A%2F%2Ftotp%2Ftest%40example.com%3Fsecret%3D#{secret}&chs=200x200"
        end
      end
      ...
      context 'qr codes' do
        ...
        it { should eq "https://image-charts.com/chart?cht=qr&chl=otpauth%3A%2F%2Ftotp%2Ftest%40example.com%3Fsecret%3D#{secret}&chs=200x200" }
        ...
        context 'custom proc' do
          ...
          it { should eq "https://image-charts.com/chart?cht=qr&chl=otpauth%3A%2F%2Ftotp%2Ftest_user%40futureadvisor-admin%3Fsecret%3D#{secret}&chs=200x200" }
        end

        context 'custom issuer' do
          let(:user) { UserFactory.create ProcIssuerUser }
          it { should eq "https://image-charts.com/chart?cht=qr&chl=otpauth%3A%2F%2Ftotp%2FFA%2520Admin%3Atest%40example.com%3Fsecret%3D#{secret}%26issuer%3DFA%2BAdmin&chs=200x200" }
        end
        ...
        context 'custom qr size' do
          ...
          it { should eq "https://image-charts.com/chart?cht=qr&chl=otpauth%3A%2F%2Ftotp%2Ftest%40example.com%3Fsecret%3D#{secret}&chs=300x300" }
        end

        context 'qr size passed to method' do
          subject { user.google_qr_uri('400x400') }
          let(:user) { UserFactory.create StringUser }
          it { should eq "https://image-charts.com/chart?cht=qr&chl=otpauth%3A%2F%2Ftotp%2Ftest%40example.com%3Fsecret%3D#{secret}&chs=400x400" }
        end
